...
cl_copula_gaussian_b <- function(x,
                                 y,
                                 var_equal = FALSE,
                                 laplace = 0,
                                 sd_thresh = 1e-8,
  ...
  n_classes <- sapply(class_names, function(m)
    sum(y == m))
  ...
  for (i in 1:k_class) {
    y_new[y_new == class_names[i]] <-
      paste0(class_names[i], "_", m_cluster_list[[i]]$classification)
    class_names_new[[i]] <-
      paste0(class_names[i], "_", 1:m_cluster_list[[i]]$G)
  }
  y_new <-
    factor(y_new,
           levels = unlist(class_names_new),
           labels = unlist(class_names_new))

  results <- copula_gaussian_b(
    x = x,
    y = y_new,
    laplace = laplace,
    sd_thresh = sd_thresh,
