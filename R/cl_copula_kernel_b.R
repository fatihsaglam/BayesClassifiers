#' @title  Clustered Copula Bayes Classifier with Kernel marginals
#'
#' @description Clustered Copula Bayes Classifier with Kernel marginals.
#'
#' @param x asd
#' @param y asd
#' @param sd_thresh = 1e-8,
#' @param laplace asd
#' @param bw_thresh asd
#' @param bw_method asd
#' @param bw_adjust asd
...
#'
#' @rdname cl_copula_kernel_b
#' @export

cl_copula_kernel_b <- function(
    x,
    y,
    sd_thresh = 1e-8,
    laplace = 0,
  ...
  results <- copula_kernel_b(x = x, y = y_new, sd_thresh = sd_thresh,
                             laplace = laplace, bw_thresh = bw_thresh,
  ...
  class(results) <- "cl_copula_kernel_b"
  return(results)
}
