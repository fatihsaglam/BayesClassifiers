#' @title  Multivariate Kernel Bayes Classifier
#'
#' @description Multivariate Kernel Bayes Classifier.
#'
#' @param x asd
#' @param y asd
#' @param laplace asd
...
mvkernel_b <- function(
  ...
  ## numerical variables
  pars_numeric <- list()
  f_bw <- get(paste0("H", bw_method))

  if (p_numerics > 0) {
    for (i in 1:k_class) {
      pars_numeric[[i]] <- list()
      pars_numeric[[i]]$bw <- f_bw(x_classes_numerics[[i]])*bw_adjust
